...
function anexo(event) {
    nomeCandidato = document.getElementById("nomeCandidato").value.trim()
    if(nomeCandidato.length != 0){
        if(document.getElementById("areaCandidato").value.length > 3 ){
            try {
                const acao = event.currentTarget.getAttribute("data-acao");
                const inputFile = $(event.currentTarget).parent().parent().find(".inputAnexo")[0]
                document.getElementById("fdCurriculoCandidato").value = "Currículo - " + nomeCandidato
                const fileDescription = "Currículo - " + nomeCandidato
                if (acao == "upload") {
            ...
            FLUIGC.message.alert({
                message: 'Antes de inserir o currículo preencha a área de atuação.',
                title: 'Mensagem',
                label: 'OK'
            }, function(el, ev) {
        ...
        FLUIGC.message.alert({
            message: 'Antes de inserir o currículo preencha o nome completo.',
            title: 'Mensagem',
            label: 'OK'
        }, function(el, ev) {
            //Callback action executed by the user...
...
$(function () {
    try {
        window.parent.$("#ecm-navigation-inputFile-clone").on('change', function (e) {
            ...
            if (fileDescription && fileDescription) {
                ...
                if (getMode() == "MOD") {
                    btnState(inputNameFile, 'viewer', 'download');
                }
            }
        });
    } catch (e) {
    ...
    document.getElementById("fnCurriculoCandidato").value = document.getElementById("nomeCandidato").value
});
